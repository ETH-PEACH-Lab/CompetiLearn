...
def stream():
    ...
    if mode == 'rag_with_link':
        result = Response(stream_with_context(get_query_result_rag_stream(query, search_mode, temperature, return_source=True)), content_type='text/event-stream')
    elif mode == 'rag_without_link':
...
if __name__ == '__main__':
    app.run(debug=True, host='0.0.0.0', port=5001)  # Ensure it runs on 0.0.0.0 to be accessible from outside the container
